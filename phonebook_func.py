...
def center_window(self, w, h): # pass in the tkinter frame (master) reference and the w and h
    ...
    # calculate x and y coordinates to paint the app centeredon the user's screen
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_phonebook (col_fname, col_lname, col_fullname, col_phone, col_phone, col_email) VALUES (?,?,?,?,?)""",)
            conn.commit()
    conn.close()
...
def onSelect(self,event):
    ...
    with conn:
        ...
        # this returns a tuple and we can since it into 4 parts using data[] during the irteration
        for data in varBody:
...
def addToList(self):
    ...
    var_lname = self.txtlname.get()
    # normalize the data to keep it consistent in the database
    var_fname = var_fname.strip() # this will remove the blank spaces before and after the user's entries
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0):
        ...
        with conn:
            ...
            chkName = count
            if chkName == 0: # if there is = then there is no existance of the fullname and we can add new data
                ...
                onClear(self) # calll the function to clear all of the textboxes
            else:
                messagebox.showerror("Name Error","'{}' already exists in the database! Please choose a different name.".format(var_fullname))
                onClear(self) # call the functuon to clear all of the textboxes
        conn.commit()
        conn.close()
    else:
...
def onUpdate(self):
    try:
        var_select = selflstList1.curselection()[0]
        var_value = self.lstList1.get(var_select)
    except:
        messagebox("Missing selection", "No name was selected from the list box. \nCancelling the Update request.")
        return
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        conn = sqlite.connect('phonebook')
        with conn:
            cur = conn.cursor()
            # count records to see if the user's changes are already in
            # the datahbase meaning, there are no changes to update
